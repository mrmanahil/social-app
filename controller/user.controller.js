...
exports.updatUser = async (req, res) => {
  try {
    ...
    if (verifyUser.id === req.body.id) {
      ...
      try {
        const user = await User.findByIdAndUpdate(verifyUser.id, {
          $set: req.body,
        });
        console.log(user);
...
exports.requestUser = async (req, res) => {
  ...
  const sendReq = await Request.findOne({
    $or: [
      { receiverId: verifyUser.id, senderId: req.body.id },
      { receiverId: req.body.id, senderId: verifyUser.id },
    ],
  });
  ...
  try {
    if (verifyUser.id == req.body.id)
      return res.send({
        status: 200,
        success: false,
        message: "you cant request yourself",
      });
    ...
    if (!sendReq) {
      const user = await Request.create({
        senderId: verifyUser.id,
        receiverId: req.params.id,
        sendTo: verifyUser.id,
      });
      res.send({ status: 200, success: true, message: user });

      const serverKey =
        "BEUid7sVFoxS1Fv4p82UMBkT7uf5xe3gciYTyMhcnvU3cU-Pt_A6TDfYaRS7wq4FdkAjy8rVZSc56Uqa66cNAB4"; //put your server key here
      ...
      if (sendReq.status == "Pending")
        return res.send({
          status: 200,
          success: false,
          message: "Status already in pending",
        });

      if (sendReq.status == "Accepted")
        return res.send({
          status: 200,
          success: false,
      ...
      const newRecord = await Request.findOneAndUpdate(
        { _id: sendReq._id },
        { status: "Pending" }
      );
      res.send({
...
exports.acceptRequest = async (req, res) => {
  const token = req.headers["x-api-token"];
  const verifyUser = await jwt.verify(token, process.env.JWT_TOKEN);
  const accptReq = await Request.findOne({
    $or: [
      { receiverId: verifyUser.id, senderId: req.body.id },
      { receiverId: req.body.id, senderId: verifyUser.id },
    ],
  ...
  try {
    ...
    if (verifyUser.id == accptReq.sendTo)
      return res.send({
        status: 200,
        success: false,
    ...
    if (accptReq.status !== "Accepted") {
      const acceptReq = await Request.findOneAndUpdate(
        { _id: accptReq._id },
        { status: "Accepted" }
      );
...
exports.denyRequest = async (req, res) => {
  const token = req.headers["x-api-token"];
  const verifyUser = await jwt.verify(token, process.env.JWT_TOKEN);
  const denyReq = await Request.findOne({
    $or: [
      { receiverId: verifyUser.id, senderId: req.body.id },
      { receiverId: req.body.id, senderId: verifyUser.id },
    ],
  ...
  try {
    ...
    if (verifyUser.id == denyReq.sendTo)
      return res.send({
        status: 200,
        success: false,
        message: "User is not valid",
    ...
    if (denyReq.status !== null) {
      const updatReq = await Request.findOneAndUpdate(
        { _id: denyReq._id },
        { status: null }
      );
    ...
    res.send({
      status: 200,
      success: false,
      message: "Request is already deny",
    });
  } catch (err) {
    console.log(`Accept request error: ${err}`);
  }
